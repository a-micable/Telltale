...
public:
    ...
    Result register_handler(uint16_t type_id, HandlerEntry::HandlerFn fn,
                            void* context, uint16_t handler_id);
    Result deregister_handler(uint16_t type_id);
    Result replace_handler(uint16_t type_id, HandlerEntry::HandlerFn fn,
                           void* context, uint16_t handler_id);
    ...
    using HandlerResolverFn = std::function<std::pair<HandlerEntry::HandlerFn, void*>(uint16_t handler_id)>;
