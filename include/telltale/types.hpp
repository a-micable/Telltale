...
enum class ReplayMode { Execute, VerifyOnly };
...
struct HandlerEntry {
  using HandlerFn = void (*)(uint16_t type_id, const uint8_t* payload, size_t payload_len,
                             void* context);
  ...
  HandlerEntry() : function(nullptr), context(nullptr), handler_id(0), active(false) {}

  HandlerEntry(HandlerFn fn, void* ctx, uint16_t id)
      : function(fn), context(ctx), handler_id(id), active(fn != nullptr) {}

  bool is_valid() const { return active && function != nullptr; }
};
...
struct FileHeader {
  ...
  FileHeader() : version(0), record_count(0), header_crc(0) { std::memcpy(magic, MAGIC, 4); }
};
...
inline const char* event_type_name(uint16_t type_id) {
  switch (type_id) {
    ...
    case static_cast<uint16_t>(EventType::Stats):
      return "Stats";
    case static_cast<uint16_t>(EventType::SchemaUpdate):
      return "SchemaUpdate";
    default:
      return "Unknown";
  }
...
inline int64_t read_i64_le(const uint8_t* data) { return static_cast<int64_t>(read_u64_le(data)); }
