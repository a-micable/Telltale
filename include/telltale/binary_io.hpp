...
public:
    ...
    static std::vector<uint8_t> encode_counter_payload(const std::string& name,
                                                       int64_t delta, bool absolute);
    static std::vector<uint8_t> encode_keyvalue_payload(const std::string& key,
                                                          int64_t value, bool overwrite);
    ...
    static std::vector<uint8_t> encode_batch_payload(const std::vector<std::vector<uint8_t>>& sub_payloads);
...
public:
    ...
    static Result decode_string_payload(const std::vector<uint8_t>& payload,
                                         size_t offset, std::string& out);
    static Result decode_counter_payload(const std::vector<uint8_t>& payload,
                                          CounterDelta& out);
    static Result decode_keyvalue_payload(const std::vector<uint8_t>& payload,
                                           KeyValueEntry& out);
    static Result decode_timestamp_payload(const std::vector<uint8_t>& payload,
                                            TimestampMarker& out);
    static Result decode_checksum_payload(const std::vector<uint8_t>& payload,
                                           ChecksumSpec& out);
    static Result decode_batch_header(const std::vector<uint8_t>& payload,
                                       BatchHeader& out);
    static Result decode_reset_payload(const std::vector<uint8_t>& payload,
                                        uint8_t& scope_flags);
    static Result decode_print_payload(const std::vector<uint8_t>& payload,
                                        PrintMessage& out);
    static Result decode_stats_payload(const std::vector<uint8_t>& payload,
                                      StatsRequest& out);
...
private:
    ...
    Result validate_record_crc(uint16_t type_id,
