...
public:
    ...
    Jitter& operator=(Jitter&&) = default;
...
// Utility functions for jitter
namespace {

uint64_t compute_hash_jitter(const void* data, size_t size) {
...
bool validate_jitter(const std::vector<uint8_t>& data) {
...
std::vector<uint8_t> transform_jitter(const std::vector<uint8_t>& input) {
