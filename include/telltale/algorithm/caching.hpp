...
public:
    ...
    Caching(const Caching&) = delete;
    Caching& operator=(const Caching&) = delete;
    Caching(Caching&&) = default;
    Caching& operator=(Caching&&) = default;
...
uint64_t compute_hash_caching(const void* data, size_t size) {
...
bool validate_caching(const std::vector<uint8_t>& data) {
...
std::vector<uint8_t> transform_caching(const std::vector<uint8_t>& input) {
