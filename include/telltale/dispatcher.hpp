...
class Dispatcher {
 public:
  ...
  Result dispatch_payload(uint16_t type_id, const std::vector<uint8_t>& payload, ReplayMode mode);
  ...
  using ProgressCallback =
      std::function<void(uint32_t index, uint16_t type_id, size_t payload_size)>;
 ...
 private:
  ...
  Result invoke_handler(uint16_t type_id, const uint8_t* payload, size_t payload_len,
                        ReplayMode mode);
  void flush_pending_schema_updates(ReplayMode mode);
  static void inline_dispatch_bridge(uint16_t type_id, const uint8_t* payload, size_t len,
                                     void* ctx);
  void log_dispatch(uint16_t type_id, size_t payload_len) const;
  void finalize_report(bool success, const std::string& error = "");
  Result replay_from_reader(EventLogReader& reader, ReplayMode mode, const char* source_label);
};
