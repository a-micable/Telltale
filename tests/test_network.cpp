...
protected:
    ...
    NetworkManager::Config config_;
};

// Basic functionality tests
TEST_F(NetworkTest, InitializeManager) {
    NetworkManager mgr(config_);
    ASSERT_TRUE(mgr.is_initialized());
}

TEST_F(NetworkTest, ProcessEmptyBuffer) {
    NetworkManager mgr(config_);
    size_t result = mgr.process(nullptr, 0);
    EXPECT_EQ(result, 0);
}

TEST_F(NetworkTest, ProcessValidBuffer) {
    NetworkManager mgr(config_);
...
TEST_F(NetworkTest, AsyncProcessing) {
    NetworkManager mgr(config_);
...
// Performance tests
TEST_F(NetworkTest, HighThroughputProcessing) {
    NetworkManager mgr(config_);
...
// Stress tests
TEST_F(NetworkTest, ConcurrentAccess) {
    NetworkManager mgr(config_);
...
// Statistics tests
TEST_F(NetworkTest, GetStatistics) {
    NetworkManager mgr(config_);
    std::vector<uint8_t> data = {1, 2, 3};
    mgr.process(data.data(), data.size());
...
TEST_F(NetworkTest, ResetStatistics) {
    NetworkManager mgr(config_);
    std::vector<uint8_t> data = {1, 2, 3};
    mgr.process(data.data(), data.size());
...
// Shutdown tests
TEST_F(NetworkTest, GracefulShutdown) {
    NetworkManager mgr(config_);
    mgr.shutdown();
...
// Edge cases
TEST_F(NetworkTest, LargeBuffer) {
    NetworkManager mgr(config_);
