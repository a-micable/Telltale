...
class RecoveryTest : public ::testing::Test {
 protected:
  ...
  RecoveryManager::Config config_;
};

// Basic functionality tests
TEST_F(RecoveryTest, InitializeManager) {
  RecoveryManager mgr(config_);
  ASSERT_TRUE(mgr.is_initialized());
}

TEST_F(RecoveryTest, ProcessEmptyBuffer) {
  RecoveryManager mgr(config_);
  size_t result = mgr.process(nullptr, 0);
  EXPECT_EQ(result, 0);
}

TEST_F(RecoveryTest, ProcessValidBuffer) {
  RecoveryManager mgr(config_);
...
TEST_F(RecoveryTest, AsyncProcessing) {
  RecoveryManager mgr(config_);
...
// Performance tests
TEST_F(RecoveryTest, HighThroughputProcessing) {
  RecoveryManager mgr(config_);
...
// Stress tests
TEST_F(RecoveryTest, ConcurrentAccess) {
  RecoveryManager mgr(config_);
...
// Statistics tests
TEST_F(RecoveryTest, GetStatistics) {
  RecoveryManager mgr(config_);
  std::vector<uint8_t> data = {1, 2, 3};
  mgr.process(data.data(), data.size());
...
TEST_F(RecoveryTest, ResetStatistics) {
  RecoveryManager mgr(config_);
  std::vector<uint8_t> data = {1, 2, 3};
  mgr.process(data.data(), data.size());
...
// Shutdown tests
TEST_F(RecoveryTest, GracefulShutdown) {
  RecoveryManager mgr(config_);
  mgr.shutdown();
...
// Edge cases
TEST_F(RecoveryTest, LargeBuffer) {
  RecoveryManager mgr(config_);
