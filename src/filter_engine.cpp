...
TimeRangeFilter::TimeRangeFilter()
    : min_epoch_millis(0), max_epoch_millis(UINT64_MAX),
      enabled(false), use_record_index_fallback(true) {}
...
static bool compare_string_op(FilterFieldOp op, const std::string& actual, const std::string& expected) {
    switch (op) {
        case FilterFieldOp::Equal: return actual == expected;
        case FilterFieldOp::NotEqual: return actual != expected;
        case FilterFieldOp::Contains: return actual.find(expected) != std::string::npos;
...
static bool compare_int_op(FilterFieldOp op, int64_t actual, int64_t expected) {
    switch (op) {
        case FilterFieldOp::Equal: return actual == expected;
        case FilterFieldOp::NotEqual: return actual != expected;
        case FilterFieldOp::GreaterThan: return actual > expected;
        case FilterFieldOp::LessThan: return actual < expected;
        case FilterFieldOp::GreaterOrEqual: return actual >= expected;
        case FilterFieldOp::LessOrEqual: return actual <= expected;
        default: return false;
    }
}
...
bool field_matches(const PayloadFieldFilter& filter, const DecodedFields& fields) {
    ...
    if (ii != fields.int_fields.end()) {
        int64_t expected = filter.use_int_value ? filter.int_value :
...
bool PayloadFieldFilter::matches(uint16_t type_id,
...
void FilterEngine::set_criteria(const FilterCriteria& criteria) {
    criteria_ = criteria;
}
...
void FilterEngine::set_match_callback(MatchCallback cb) {
    match_callback_ = std::move(cb);
}

std::string FilterEngine::field_op_name(FilterFieldOp op) {
    switch (op) {
        case FilterFieldOp::Equal: return "eq";
        case FilterFieldOp::NotEqual: return "ne";
        case FilterFieldOp::GreaterThan: return "gt";
        case FilterFieldOp::LessThan: return "lt";
        case FilterFieldOp::GreaterOrEqual: return "ge";
        case FilterFieldOp::LessOrEqual: return "le";
        case FilterFieldOp::Contains: return "contains";
        case FilterFieldOp::StartsWith: return "startswith";
        case FilterFieldOp::EndsWith: return "endswith";
        default: return "?";
    }
}
...
FilterCriteria FilterEngine::parse_criteria_from_args(const std::vector<std::string>& args) {
    ...
    for (size_t i = 0; i < args.size(); ++i) {
        if (args[i] == "--type-min" && i + 1 < args.size()) {
            ...
            c.type_range.min_type = static_cast<uint16_t>(
                std::strtoul(args[++i].c_str(), nullptr, 0));
        } else if (args[i] == "--type-max" && i + 1 < args.size()) {
            c.type_range.enabled = true;
            c.type_range.max_type = static_cast<uint16_t>(
                std::strtoul(args[++i].c_str(), nullptr, 0));
            ...
            if (args[++i] == "or") c.combine_op = FilterCombineOp::Or;
            else c.combine_op = FilterCombineOp::And;
