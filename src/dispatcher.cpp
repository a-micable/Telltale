...
void Dispatcher::inline_dispatch_bridge(uint16_t type_id, const uint8_t* payload, size_t len,
                                        void* ctx) {
...
Result Dispatcher::process_schema_update(const std::vector<uint8_t>& payload, ReplayMode mode) {
  SchemaUpdatePayload update;
  ...
  if (mode == ReplayMode::VerifyOnly) {
    ...
    ++verify_only_skipped_;
    if (verbose_ && output_) {
      *output_ << "[verify] schema update: " << SchemaUpdateCodec::flags_to_string(update.flags)
...
Result Dispatcher::invoke_handler(uint16_t type_id, const uint8_t* payload, size_t payload_len,
                                  ReplayMode mode) {
  if (mode == ReplayMode::VerifyOnly) {
  ...
  if (entry == nullptr) {
    ...
    registry_.invoke_no_handler(type_id);
    if (verbose_ && output_) {
      *output_ << "[warn] no handler for type 0x" << std::hex << type_id << std::dec << std::endl;
    }
    return Result::success();
  }
...
Result Dispatcher::dispatch_payload(uint16_t type_id, const std::vector<uint8_t>& payload,
                                    ReplayMode mode) {
...
Result Dispatcher::replay_from_reader(EventLogReader& reader, ReplayMode mode,
  ...
  if (verbose_ && output_) {
    *output_ << "Replaying " << source_label << " (" << header.record_count << " records)"
             << std::endl;
  }
  ...
  if (index != header.record_count) {
    std::string msg = "Record count mismatch: header says " + std::to_string(header.record_count) +
