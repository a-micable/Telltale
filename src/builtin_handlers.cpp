...
void ReplayState::print_stats(std::ostream& out, uint8_t output_flags,
  ...
  if (output_flags & 0x08) {
    ...
    out << pfx << "  events=" << event_sequence << " resets=" << reset_count
...
ReplayContext::ReplayContext()
    : output_(nullptr),
      null_buf_(),
      default_output_(&null_buf_),
      verbose_(false),
      dispatch_depth_(0),
      total_events_handled_(0),
      registry_(nullptr),
      inline_dispatch_(nullptr),
      inline_dispatch_ctx_(nullptr) {}
...
static ReplayContext* get_context(void* ctx) { return static_cast<ReplayContext*>(ctx); }
...
void handle_counter(uint16_t /*type_id*/, const uint8_t* payload, size_t payload_len,
  ...
  if (delta.use_absolute) {
    ...
    if (ctx->verbose()) {
      ctx->output() << "[counter] " << delta.name << " += " << delta.delta << " (now "
                    << ctx->state().get_counter(delta.name) << ")" << std::endl;
    }
  }
}
...
void handle_checksum(uint16_t /*type_id*/, const uint8_t* payload, size_t payload_len,
  ...
  if (ctx->verbose()) {
    ctx->output() << "[checksum] " << spec.label << ": expected=0x" << std::hex << spec.expected_crc
                  << " computed=0x" << computed << std::dec << (match ? " OK" : " MISMATCH")
                  << std::endl;
  }
}

void handle_batch(uint16_t /*type_id*/, const uint8_t* payload, size_t payload_len, void* context) {
  ReplayContext* ctx = get_context(context);
  if (!ctx) return;
  std::vector<uint8_t> vec(payload, payload + payload_len);
  BatchHeader header;
...
void handle_reset(uint16_t /*type_id*/, const uint8_t* payload, size_t payload_len, void* context) {
  ReplayContext* ctx = get_context(context);
  if (!ctx) return;
  std::vector<uint8_t> vec(payload, payload + payload_len);
  uint8_t scope = 0xFF;
  ...
  if (ctx->verbose()) {
    ctx->output() << "[reset] scope=0x" << std::hex << static_cast<int>(scope) << std::dec
                  << std::endl;
  }
}

void handle_print(uint16_t /*type_id*/, const uint8_t* payload, size_t payload_len, void* context) {
  ReplayContext* ctx = get_context(context);
  if (!ctx) return;
  std::vector<uint8_t> vec(payload, payload + payload_len);
  PrintMessage msg;
...
void handle_stats(uint16_t /*type_id*/, const uint8_t* payload, size_t payload_len, void* context) {
  ReplayContext* ctx = get_context(context);
  if (!ctx) return;
  std::vector<uint8_t> vec(payload, payload + payload_len);
  StatsRequest req;
...
void register_builtin_handlers(HandlerRegistry& registry, ReplayContext* context) {
  ...
  registry.register_handler(static_cast<uint16_t>(EventType::Counter), handle_counter, context,
                            static_cast<uint16_t>(HandlerId::BuiltinCounter));
  registry.register_handler(static_cast<uint16_t>(EventType::KeyValue), handle_keyvalue, context,
                            static_cast<uint16_t>(HandlerId::BuiltinKeyValue));
  registry.register_handler(static_cast<uint16_t>(EventType::Timestamp), handle_timestamp, context,
                            static_cast<uint16_t>(HandlerId::BuiltinTimestamp));
  registry.register_handler(static_cast<uint16_t>(EventType::Checksum), handle_checksum, context,
                            static_cast<uint16_t>(HandlerId::BuiltinChecksum));
  registry.register_handler(static_cast<uint16_t>(EventType::Batch), handle_batch, context,
                            static_cast<uint16_t>(HandlerId::BuiltinBatch));
  registry.register_handler(static_cast<uint16_t>(EventType::Reset), handle_reset, context,
                            static_cast<uint16_t>(HandlerId::BuiltinReset));
  registry.register_handler(static_cast<uint16_t>(EventType::Print), handle_print, context,
                            static_cast<uint16_t>(HandlerId::BuiltinPrint));
  registry.register_handler(static_cast<uint16_t>(EventType::Stats), handle_stats, context,
...
HandlerEntry::HandlerFn resolve_builtin_handler(uint16_t handler_id, ReplayContext** out_context) {
  ...
  switch (handler_id) {
    case static_cast<uint16_t>(HandlerId::BuiltinCounter):
      return handle_counter;
    case static_cast<uint16_t>(HandlerId::BuiltinKeyValue):
      return handle_keyvalue;
    case static_cast<uint16_t>(HandlerId::BuiltinTimestamp):
      return handle_timestamp;
    case static_cast<uint16_t>(HandlerId::BuiltinChecksum):
      return handle_checksum;
    case static_cast<uint16_t>(HandlerId::BuiltinBatch):
      return handle_batch;
    case static_cast<uint16_t>(HandlerId::BuiltinReset):
      return handle_reset;
    case static_cast<uint16_t>(HandlerId::BuiltinPrint):
      return handle_print;
    case static_cast<uint16_t>(HandlerId::BuiltinStats):
      return handle_stats;
    case static_cast<uint16_t>(HandlerId::BuiltinNoOp):
      return handle_noop;
    default:
      return nullptr;
  }
