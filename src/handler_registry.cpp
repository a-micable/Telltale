...
Result HandlerRegistry::validate_type_id(uint16_t type_id) const {
  if (type_id == SCHEMA_UPDATE_TYPE) {
    return Result::fail(ErrorCode::InvalidTypeId, "Cannot register handler for schema update type");
  }
...
Result HandlerRegistry::resize_table(size_t new_size) {
  if (new_size > MAX_HANDLER_TABLE_SIZE) {
    return Result::fail(ErrorCode::HandlerTableFull, "Requested table size exceeds maximum");
  }
...
Result HandlerRegistry::replace_handler(uint16_t type_id, HandlerEntry::HandlerFn fn, void* context,
                                        uint16_t handler_id) {
  Result vr = validate_type_id(type_id);
  if (!vr.ok()) return vr;
  vr = validate_handler_fn(fn);
...
Result HandlerRegistry::apply_schema_update(const SchemaUpdatePayload& update) {
  ...
  if (update.flags & static_cast<uint8_t>(SchemaUpdateFlag::ResizeTable)) {
    if (!update.has_table_size) {
      return Result::fail(ErrorCode::InvalidSchemaUpdate, "Resize requires table size");
    }
  ...
  if (update.flags & static_cast<uint8_t>(SchemaUpdateFlag::Replace)) {
    if (!update.has_handler_id) {
      return Result::fail(ErrorCode::InvalidSchemaUpdate, "Replace requires handler ID");
    }
    ...
    return replace_handler(update.type_id, resolved.first, resolved.second, update.handler_id);
  }

  if (update.flags & static_cast<uint8_t>(SchemaUpdateFlag::Register)) {
    if (!update.has_handler_id) {
      return Result::fail(ErrorCode::InvalidSchemaUpdate, "Register requires handler ID");
    }
    if (!handler_resolver_) {
      return Result::fail(ErrorCode::InternalError, "No handler resolver configured");
    }
    ...
    return register_handler(update.type_id, resolved.first, resolved.second, update.handler_id);
  }
...
bool HandlerRegistry::has_handler(uint16_t type_id) const { return lookup(type_id) != nullptr; }
...
std::vector<uint16_t> HandlerRegistry::active_type_ids() const { return registered_type_ids(); }
...
void HandlerRegistry::reset_to_defaults() { clear_all(); }

std::vector<HandlerRegistry::RegistrationInfo> HandlerRegistry::list_registrations() const {
