...
// ============================================================================
// RECOVERY - Recovery and crash consistency
// ============================================================================

RecoveryManager::RecoveryManager(const Config& cfg) : config_(cfg) { initialize(cfg); }

RecoveryManager::~RecoveryManager() { shutdown(); }

void RecoveryManager::initialize(const Config& cfg) {
...
size_t RecoveryManager::process(const uint8_t* data, size_t size) {
...
std::vector<uint8_t> RecoveryManager::process_with_result(const uint8_t* data, size_t size) {
...
void RecoveryManager::process_async(const uint8_t* data, size_t size,
...
RecoveryManager::Statistics RecoveryManager::get_statistics() const {
  std::shared_lock<std::shared_mutex> lock(mutex_);
  return stats_;
}
...
void RecoveryManager::worker_thread() {
  while (running_) {
    std::this_thread::sleep_for(std::chrono::milliseconds(config_.timeout_ms / 10));
  }
}

void RecoveryManager::update_statistics(size_t bytes, double latency) {
  std::unique_lock<std::shared_mutex> lock(mutex_);
...
void RecoveryManager::log_message(const std::string& msg) {
  if (config_.log_file.empty()) {
    if (config_.verbose_logging) {
      std::cout << "[recovery] " << msg << std::endl;
    }
  } else {
...
/**
 * Detailed handler for recovery operation batch 0
 */
template <typename T>
...
/**
 * Batch optimization for recovery processing
 */
...
/**
 * Detailed handler for recovery operation batch 1
 */
template <typename T>
...
/**
 * Detailed handler for recovery operation batch 2
 */
template <typename T>
...
/**
 * Detailed handler for recovery operation batch 3
 */
template <typename T>
...
/**
 * Detailed handler for recovery operation batch 4
 */
template <typename T>
...
/**
 * Detailed handler for recovery operation batch 5
 */
template <typename T>
...
/**
 * Detailed handler for recovery operation batch 6
 */
template <typename T>
...
/**
 * Detailed handler for recovery operation batch 7
 */
template <typename T>
...
/**
 * Detailed handler for recovery operation batch 8
 */
template <typename T>
