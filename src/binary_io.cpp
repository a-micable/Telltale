...
EventLogWriter::EventLogWriter(const std::string& path)
    : path_(path), record_count_(0), is_open_(false),
      is_finalized_(false), header_written_(false) {}
...
Result EventLogWriter::validate_payload_size(size_t size) const {
    if (size > MAX_PAYLOAD_SIZE) {
        return Result::fail(ErrorCode::PayloadTooLarge,
            "Payload size " + std::to_string(size) + " exceeds maximum " +
            std::to_string(MAX_PAYLOAD_SIZE));
    }
    return Result::success();
}

Result EventLogWriter::write_raw_record(uint16_t type_id,
...
Result EventLogWriter::write_event(uint16_t type_id,
                                    const std::vector<uint8_t>& payload) {
...
Result EventLogWriter::write_schema_update(uint8_t flags, uint16_t type_id,
                                              uint16_t handler_id) {
    bool include_handler = SchemaUpdateCodec::requires_handler_id(flags);
    std::vector<uint8_t> payload = SchemaUpdateCodec::encode(
        flags, type_id, handler_id, include_handler);
...
std::vector<uint8_t> EventLogWriter::encode_counter_payload(const std::string& name,
                                                              int64_t delta,
                                                              bool absolute) {
...
std::vector<uint8_t> EventLogWriter::encode_keyvalue_payload(const std::string& key,
                                                                int64_t value,
                                                                bool overwrite) {
...
EventLogReader::EventLogReader(const std::string& path)
    : path_(path), buffer_mode_(false), buffer_data_(nullptr), buffer_size_(0),
      buffer_pos_(0), is_open_(false), expected_record_count_(0), records_read_(0) {}

EventLogReader::EventLogReader(const uint8_t* data, size_t length)
    : path_(), buffer_mode_(true), buffer_data_(data), buffer_size_(length),
      buffer_pos_(0), is_open_(false), expected_record_count_(0), records_read_(0) {}
...
Result EventLogReader::read_header(FileHeader& header) {
    ...
    if (computed != header.header_crc) {
        return Result::fail(ErrorCode::HeaderCrcMismatch,
            "Header CRC mismatch: expected " + std::to_string(computed) +
            ", got " + std::to_string(header.header_crc));
    }
...
Result EventLogReader::validate_record_crc(uint16_t type_id,
                                            const std::vector<uint8_t>& payload,
                                            uint32_t stored_crc) {
    uint32_t computed = Crc32::of_record(type_id, payload);
    if (computed != stored_crc) {
        return Result::fail(ErrorCode::RecordCrcMismatch,
            "Record CRC mismatch for type " + std::to_string(type_id) +
            ": expected " + std::to_string(computed) +
            ", got " + std::to_string(stored_crc));
    }
    return Result::success();
}
...
Result EventLogReader::decode_string_payload(const std::vector<uint8_t>& payload,
...
Result EventLogReader::decode_batch_header(const std::vector<uint8_t>& payload,
                                            BatchHeader& out) {
    if (payload.size() < 4) {
