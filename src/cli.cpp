...
std::string Cli::get_flag_value(const std::vector<std::string>& args,
...
void Cli::print_filter_usage() {
    ...
    std::cout << "  --field NAME OP VALUE   Payload field filter (op: eq,ne,gt,lt,contains)" << std::endl;
...
void Cli::print_compact_usage() {
    std::cout << "Usage: telltale compact <output-file> <input-file> [more-inputs...] [options]" << std::endl;
    std::cout << "Options:" << std::endl;
...
void Cli::print_import_usage() {
    std::cout << "Usage: telltale import <input-text-file> <output-binary-file> [options]" << std::endl;
    std::cout << "Options:" << std::endl;
...
std::vector<std::string> Cli::collect_positional(const std::vector<std::string>& args,
    ...
    for (size_t i = start_index; i < args.size(); ++i) {
        if (!args[i].empty() && args[i][0] == '-') {
            if (args[i] == "--verbose" || args[i] == "--explain" ||
                args[i] == "--no-schema" || args[i] == "--raw-hex" ||
                args[i] == "--lenient" || args[i] == "--no-dedupe-reset" ||
                args[i] == "--verify-output") {
                continue;
            }
            if (args[i] == "--type" || args[i] == "--type-min" || args[i] == "--type-max" ||
                args[i] == "--time-min" || args[i] == "--time-max" ||
                args[i] == "--field" || args[i] == "--combine" || args[i] == "--events") {
                ++i;
                continue;
            }
            break;
...
int generate_sample_log(const std::string& output_path, size_t event_count, bool verbose) {
...

  {
    ...
    writer.write_schema_update(
        static_cast<uint8_t>(SchemaUpdateFlag::Register),
        0x0100,
...
int Cli::cmd_replay(const std::vector<std::string>& args) {
    ...
    std::cout << "Replay complete: " << report.records_processed << " records, "
              << report.handlers_invoked << " handlers invoked, "
              << report.schema_updates << " schema updates" << std::endl;
    return 0;
}
...
int Cli::cmd_verify(const std::vector<std::string>& args) {
    ...
    const auto& report = dispatcher.last_report();
    std::cout << "Verify OK: " << report.records_processed << " records, "
              << report.schema_updates << " schema updates validated, "
              << report.missing_handlers << " unhandled types" << std::endl;
    return 0;
}
...
int Cli::cmd_filter(const std::vector<std::string>& args) {
    ...
    std::cout << "Filter: " << engine.match_count() << " of "
              << engine.total_records_loaded() << " records written to "
              << output << std::endl;
    return 0;
}
...
int Cli::cmd_compact(const std::vector<std::string>& args) {
    ...
    if (verbose) {
        std::cout << "Compacted " << stats.input_files << " files, "
                  << stats.input_records_total << " input records -> "
                  << stats.output_records << " output records, "
                  << stats.resets_deduped << " resets deduped" << std::endl;
    } else {
        std::cout << "Compact OK: " << stats.output_records << " records -> "
                  << output << std::endl;
    }
    return 0;
}
...
int Cli::cmd_export(const std::vector<std::string>& args) {
    ...
    std::cout << "Exported " << exporter.records_exported() << " records to "
              << output << std::endl;
    return 0;
}
...
int Cli::cmd_import(const std::vector<std::string>& args) {
    ...
    std::cout << "Import OK: " << importer.records().size() << " records -> "
              << output << std::endl;
    return 0;
}
