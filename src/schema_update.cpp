...
std::vector<uint8_t> SchemaUpdateCodec::encode(uint8_t flags, uint16_t type_id,
                                                  uint16_t handler_id,
...
Result SchemaUpdateCodec::validate_payload(const std::vector<uint8_t>& payload) {
    if (payload.size() < MIN_PAYLOAD_SIZE) {
        return Result::fail(ErrorCode::InvalidSchemaUpdate,
            "Schema update payload too short");
    }
...
Result SchemaUpdateCodec::decode(const std::vector<uint8_t>& payload,
