...
Crc32::Crc32() : state_(INITIAL) {}

void Crc32::reset() { state_ = INITIAL; }

void Crc32::update(const uint8_t* data, size_t length) {
  for (size_t i = 0; i < length; ++i) {
    uint8_t index = static_cast<uint8_t>((state_ ^ data[i]) & 0xFFU);
    state_ = (state_ >> 8) ^ TABLE[index];
  }
}

void Crc32::update(uint8_t byte) {
  uint8_t index = static_cast<uint8_t>((state_ ^ byte) & 0xFFU);
  state_ = (state_ >> 8) ^ TABLE[index];
}

uint32_t Crc32::finalize() const { return state_ ^ FINAL_XOR; }
...
uint32_t Crc32::of_bytes(const uint8_t* data, size_t length) {
  return Crc32().compute(data, length);
}

uint32_t Crc32::of_vector(const std::vector<uint8_t>& data) {
  return of_bytes(data.data(), data.size());
}

uint32_t Crc32::of_record(uint16_t type_id, const std::vector<uint8_t>& payload) {
  uint8_t header[6];
...
uint32_t Crc32::of_header_fields(uint16_t version, uint32_t record_count) {
  uint8_t buf[6];
  write_u16_le(buf, version);
  write_u32_le(buf + 2, record_count);
  return of_bytes(buf, 6);
}
