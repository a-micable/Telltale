...
Stream_IoManager::Stream_IoManager(const Config& cfg) 
    : config_(cfg) {
    initialize(cfg);
}

Stream_IoManager::~Stream_IoManager() {
    shutdown();
}
...
std::vector<uint8_t> Stream_IoManager::process_with_result(
...
void Stream_IoManager::process_async(
    const uint8_t* data, size_t size,
